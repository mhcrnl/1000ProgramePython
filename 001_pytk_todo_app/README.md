Here is a complete, modernized reimagining of your Tkinter script built with **Flet**.

This rewrite preserves the core mechanics of your original code—such as pressing **Enter** to submit a task and using alternating background row styles—but upgrades the interface to a sleek, responsive, and cross-platform architecture.
...
* **Asynchronous-ready UI:** Replaced Tkinter's blocking structure with an event-driven framework layout.
* **Scroll Management:** Wrapped tasks inside a managed `ListView` layout component instead of direct `.pack()` stacking, preventing items from running off-screen if the list gets long.
* **Adaptive Styling:** The app automatically shifts color syntax rules to respect system dark/light mode configurations while sustaining clean structural contrast.
